...
class Swing:
    def __init__(self, v_color, ps_color, length, mass, ang_velo, ang, friction_coefficient, external_force_amplitude, external_force_frequency, push_angle, effective_angle_diff):
        ...
        self.pendulum = sphere(pos=vector(0, -self.pendulum_length, 0), radius=1, color=color.red)
    ...
    def update(self):
        ...
        external_force = self.external_force_amplitude * cos(self.external_force_frequency * t) if abs(self.pendulum_ang) < self.push_angle else 0
        torque = (
            (self.pendulum_mass * self.gravity).cross(self.pendulum.pos)
            + self.friction_force * self.pendulum_length * vector(0, 0, -1)
            + external_force * abs(cos(self.pendulum_ang + self.effective_angle_diff)) * vector(0, 0, -1)
        )
        ...
        self.pendulum.pos = vector(
            self.pendulum_length * sin(self.pendulum_ang), -self.pendulum_length * cos(self.pendulum_ang), 0
        )
